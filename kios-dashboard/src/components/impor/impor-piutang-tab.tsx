...
function toRows(data: Piutang[]): string[][] {
  return data.map((p) => [
    p.id,
    p.tanggal,
    p.pelanggan_id,
...
const TEMPLATE_COLS = "phone,nama,pokok,dibayar,catatan,tanggal";
const TEMPLATE_EXAMPLE = "08123456789,Budi,150000,0,hutang beras,2026-06-01";

function templateCsvContent(): string {
  return `${TEMPLATE_COLS}\n${TEMPLATE_EXAMPLE}\n`;
}

export function ImporPiutangTab() {
  ...
  function apply() {
    ...
    start(async () => {
      const r = await importPiutangAction(rows);
      setResult(r);
      if (r.ok) {
        router.refresh();
      }
  ...
  function downloadTemplate() {
    ...
    a.href = url;
    a.download = "template-piutang.csv";
    a.click();
  ...
  function handleExportCsv() {
    startExport(async () => {
      const data = await exportPiutangAction();
      exportCsv("piutang.csv", HEADERS, toRows(data));
    });
  }
  ...
  function handleExportXlsx() {
    startExport(async () => {
      const data = await exportPiutangAction();
      await exportXlsx("piutang.xlsx", "Piutang", HEADERS, toRows(data));
    });
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Ekspor Piutang</CardTitle>
          <p className="mt-1 text-sm text-muted-foreground">
            Unduh semua data piutang pelanggan ke file CSV atau Excel.
          </p>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="flex-row items-start justify-between gap-3">
          <div>
            <CardTitle>Impor Piutang</CardTitle>
            <p className="mt-1 text-sm text-muted-foreground">
              Unggah Excel/CSV untuk membuat data piutang pelanggan secara massal.
            </p>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="rounded-lg border border-dashed bg-muted/30 p-3 text-xs text-muted-foreground">
            Kolom yang dikenali:{" "}
            {["phone", "nama", "pokok", "dibayar", "catatan", "tanggal"].map((c) => (
              <code key={c} className="mr-1 rounded bg-muted px-1.5 py-0.5 font-mono">
                {c}
              </code>
            ))}
            <p className="mt-1.5">Kolom <b>phone</b> dan <b>pokok</b> wajib diisi.</p>
          </div>
