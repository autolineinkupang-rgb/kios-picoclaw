...
func (t *UserTool) Execute(ctx context.Context, args map[string]any) *tools.ToolResult {
	...
	default:
		return tools.ErrorResult("Aksi user tidak dikenal.")
	}
}
...
func (t *UserTool) tambah(ctx context.Context, args map[string]any) *tools.ToolResult {
	id := argStr(args, "id")
	if id == "" {
		return tools.ErrorResult("ID Telegram pengguna wajib diisi.")
	}
	u := &UserKios{
		Phone:       id,
	...
	if err := t.store.SetUser(ctx, u); err != nil {
		return tools.ErrorResult("Gagal simpan pengguna.").WithError(err)
	}
...
func (t *UserTool) list(ctx context.Context) *tools.ToolResult {
	...
	if err != nil {
		return tools.ErrorResult("Gagal baca pengguna.").WithError(err)
	}
	if len(users) == 0 {
...
func (t *UserTool) setAktif(ctx context.Context, args map[string]any, aktif bool) *tools.ToolResult {
	...
	if u == nil {
		return tools.NewToolResult("Pengguna tidak ditemukan.")
	}
	u.Aktif = aktif
	if err := t.store.SetUser(ctx, u); err != nil {
		return tools.ErrorResult("Gagal simpan pengguna.").WithError(err)
...
func (t *UserTool) setRole(ctx context.Context, args map[string]any) *tools.ToolResult {
	...
	if u == nil {
		return tools.NewToolResult("Pengguna tidak ditemukan. Tambahkan dulu dengan action tambah.")
	}
